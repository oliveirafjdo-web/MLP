...
const OAUTH_BASE='https://auth.mercadolibre.com.br';
...
export async function apiGet(path,accessToken,params={}){
  const {data}=await axios.get(`${API_BASE}${path}`,{headers:{Authorization:`Bearer ${accessToken}`},params});
  return data;
}
...
export async function itemSummary(accessToken,itemId){
  ...
  try{
    const v = await apiGet(`/visits/items`, accessToken, {
      ids:itemId, date_from:new Date(Date.now()-30*864e5).toISOString()
    });
  ...
  try{
    ...
    const orders = await apiGet(`/orders/search`, accessToken, {
      order_status:'paid', tags:'paid', date_created_from:dateFrom, date_created_to:dateTo, item:itemId
    });
    orders30 = orders?.paging?.total ?? orders?.results?.length ?? null;
  }catch{}

  // rank estimation in category (default search relevance). We scan pages until found or cap.
  let rank=null, total=null;
  try{
    const cat=item.category_id;
    let offset=0, limit=50, pos=null, count=null;
    while(offset<1000 && pos===null){ // cap to 1000 results
  ...
  return {
    id:item.id, title:item.title, price,
    available_quantity:available, sold_quantity:sold,
    date_created:item.date_created, last_updated:item.last_updated,
    visits_30d:visits30, orders_30d:orders30, conversion_30d:conversion, revenue_30d:revenue30,
    seller_permalink, seller_nickname,
